...
fn validate_managed_interval(name: &str, seconds: Option<u64>) -> Result<Option<Duration>, String> {
    let Some(seconds) = seconds else {
        return Ok(None);
    };
    if !(MIN_MANAGED_INTERVAL_SECS..=MAX_MANAGED_INTERVAL_SECS).contains(&seconds) {
        return Err(format!(
            "{name} must be between {MIN_MANAGED_INTERVAL_SECS} and              {MAX_MANAGED_INTERVAL_SECS} seconds (got {seconds})"
        ));
    }
...
fn last_rejected_settings() -> &'static std::sync::Mutex<Option<RejectedManagedSettings>> {
    static LAST: std::sync::OnceLock<std::sync::Mutex<Option<RejectedManagedSettings>>> =
